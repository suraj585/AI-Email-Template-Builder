  ...
  const data = await (0, import_utils.bigBrainAPI)({
    ...
    data: {
      projectSelection: await (0, import_api.projectSelection)(
        ctx,
        (await (0, import_utils.getConfiguredDeployment)(ctx)).name,
        options.configuredDeployKey
      ),
      identifier: previewName,
      partitionId: options.partitionId ? parseInt(options.partitionId) : void 0
    }
  });
  const previewAdminKey = data.adminKey;
  const previewUrl = data.instanceUrl;
  await (0, import_deploy2.runCommand)(ctx, { ...options, url: previewUrl });
  const pushOptions = {
...
async function deployToExistingDeployment(ctx, options) {
  ...
  await (0, import_deploy2.deployToDeployment)(ctx, { url, adminKey }, options);
}
