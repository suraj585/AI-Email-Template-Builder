...
const disableLocalDeployments = new import_extra_typings.Command("disable-local-deployments").description(
  ...
  const { type } = await (0, import_utils.getConfiguredDeployment)(ctx);
  if (type !== "local") {
    (0, import_context.logFinishedStep)(ctx, "Local development is already not being used.");
    return;
  }
  await (0, import_configure.deploymentCredentialsOrConfigure)(ctx, null, {
    prod: false,
    localOptions: {
      forceUpgrade: false
    },
