...
async function pushConfig(ctx, config, adminKey, url, pushMetrics, schemaId, bundledModuleInfos) {
  ...
  try {
    ...
    if (data?.code === "AuthConfigMissingEnvironmentVariable") {
      ...
      const message2 = `Environment variable ${import_chalk.default.bold(
        variableName
  ...
  to set it up. `;
    ...
    if (data?.code === "InternalServerError") {
      ...
      if (configuredDeployment?.startsWith("local-")) {
        (0, import_errors.printLocalDeploymentOnError)(ctx);
        return ctx.crash({
          exitCode: 1,
          errorType: "fatal",
    ...
    (0, import_context.logFailure)(ctx, message);
