...
async function _ensureBackendBinaryDownloaded(ctx, version) {
  (0, import_context.logVerbose)(ctx, `Ensuring backend binary downloaded for version ${version}`);
    ...
    while (nextUrl) {
      ...
      for (const release of releases) {
        if (!latestVersion && !release.prerelease && !release.draft) {
          ...
          (0, import_context.logVerbose)(ctx, `Latest stable version is ${latestVersion}`);
        }
...
async function downloadBinary(ctx, version) {
  ...
  if (!isNaN(contentLength) && contentLength !== 0 && process.stdout.isTTY) {
    progressBar = (0, import_context.startLogProgress)(
      ctx,
  ...
  await import_fs.nodeFs.writeFileStream(zipLocation, readable, (chunk) => {
    if (progressBar !== null) {
  ...
  if (progressBar) {
    ...
    (0, import_context.logFinishedStep)(ctx, "Downloaded Convex backend binary");
  }
...
async function assertLocalBackendRunning(ctx, args) {
  (0, import_context.logVerbose)(ctx, `Checking local backend at ${args.url} is running`);
  try {
...
async function ensureBackendRunning(ctx, args) {
  ...
  while (timeElapsedSecs <= args.maxTimeSecs) {
    if (!hasShownWaiting && timeElapsedSecs > 2) {
      (0, import_context.logMessage)(ctx, "waiting for local backend to start...");
