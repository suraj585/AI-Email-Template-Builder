...
async function deploymentCredentialsOrConfigure(ctx, chosenConfiguration, cmdOptions, partitionId) {
  ...
  const deploymentOptions = cmdOptions.prod ? { kind: "prod" } : devDeployment === "local" ? { kind: "local", ...cmdOptions.localOptions } : { kind: "dev" };
  const {
    deploymentName,
    deploymentUrl: url,
    adminKey
...
async function selectProject(ctx, chosenConfiguration, cmdOptions) {
  let result = null;
  const forceDevDeployment = cmdOptions.cloud ? "cloud" : cmdOptions.local ? "local" : void 0;
...
async function getConfiguredProjectSlugs(ctx) {
  const { name: deploymentName, type } = await (0, import_utils.getConfiguredDeployment)(ctx);
