...
async function doEsbuild(ctx, dir, entryPoints2, generateSourceMaps, platform, chunksFolder, externalPackages, extraConditions) {
  ...
  try {
    ...
    return await ctx.crash({
      ...
      printedMessage: recommendUseNode ? `It looks like you are using Node APIs from a file without the "use node" directive.
See https://docs.convex.dev/functions/runtimes#nodejs-runtime` : null
    });
  }
}
...
async function entryPoints(ctx, dir) {
  ...
  for (const { isDir, path: fpath, depth } of walkDir(ctx.fs, dir)) {
    ...
    if (!ENTRY_POINT_EXTENSIONS.some((ext) => relPath.endsWith(ext))) {
      (0, import_context.logVerbose)(ctx, import_chalk.default.yellow(`Skipping non-JS file ${fpath}`));
